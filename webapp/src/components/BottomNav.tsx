      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            key={tab.id}
            type="button"
            className={`nav-tab-mini ${isActive ? 'nav-tab-mini--active' : ''}`}
          ...
          >
            {isActive && (
              <motion.div
                className="nav-tab-mini__indicator"
                layoutId="nav-indicator"
                aria-hidden="true"
                transition={{ type: 'spring', damping: 25, stiffness: 240 }}
              />
            )}
            <div className="nav-tab-mini__icon" aria-hidden="true">{tab.icon}</div>
            <span className="nav-tab-mini__label">{tab.label}</span>
          </button>
        )
      })}
    </motion.nav>
