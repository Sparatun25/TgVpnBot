...
from core.metrics import notifications_sent_total
...
async def send_trial_24h_notification(bot: Bot) -> None:
    ...
    Ищет пользователей с активным триалом, у которых осталось от (N-1) до N часов
    (где N = settings.notification_24h_before), и которые ещё не получали это уведомление.
    """
    try:
        now = datetime.now(timezone.utc)
        hours = settings.notification_24h_before
        time_Nh = now + timedelta(hours=hours)
        time_N1h = now + timedelta(hours=hours - 1)

        async with async_session_factory() as session:
            # Ищем активные триалы, которые истекают в диапазоне N-1 .. N часов
            query = (
                select(User)
                .join(Subscription)
                .where(
                    ...
                    Subscription.expires_at >= time_N1h,
                    Subscription.expires_at <= time_Nh,
            ...
            for user in users:
                try:
                    ...
                    # Метрика: уведомление физически доставлено пользователю.
                    # Считаем после успешного send_message (не в finally) — если
                    # Telegram вернул ошибку, уведомление не доставлено и инкрементить нечего.
                    notifications_sent_total.labels(type="trial_24h").inc()
        ...
        # APScheduler при серии failures отключает job (misfire policy).
        # Логируем и НЕ пробрасываем — следующий проход через минуту попробует снова.
        logger.exception("scheduler_job_failed", job="trial_24h_notification")
...
async def send_trial_1h_notification(bot: Bot) -> None:
    ...
    Ищет пользователей с активным триалом, у которых осталось (N*60-1)..N*60 минут
    (где N = settings.notification_1h_before в часах), и которые ещё не получали это уведомление.
    """
    try:
        now = datetime.now(timezone.utc)
        hours = settings.notification_1h_before
        time_Nh = now + timedelta(hours=hours)
        time_Nh_minus_1m = now + timedelta(hours=hours, minutes=-1)

        async with async_session_factory() as session:
            # Ищем активные триалы, которые истекают в диапазоне N*60-1 .. N*60 минут
            query = (
                select(User)
                .join(Subscription)
                .where(
                    Subscription.is_active == True,
                    Subscription.plan_type == PlanType.TRIAL,
                    Subscription.expires_at >= time_Nh_minus_1m,
                    Subscription.expires_at <= time_Nh,
            ...
            for user in users:
                try:
                    ...
                    notifications_sent_total.labels(type="trial_1h").inc()
        ...
        logger.exception("scheduler_job_failed", job="trial_1h_notification")
...
async def cleanup_expired_subscriptions(bot: Bot) -> None:
    ...
    try:
                ...
                if revoked:
                    ...
                    for sub in revoked:
                        try:
                            ...
                            notifications_sent_total.labels(type="expired").inc()
        ...
        logger.exception("scheduler_job_failed", job="cleanup_expired_subscriptions")
...
async def send_inactive_key_notifications(bot: Bot) -> None:
    ...
    try:
        ...
        async with async_session_factory() as session:
            ...
            for user in users_15m:
                try:
                    ...
                    notifications_sent_total.labels(type="inactive_15m").inc()
            ...
            for user in users_3h:
                try:
                    ...
                    notifications_sent_total.labels(type="inactive_3h").inc()
            ...
            for user in users_24h:
                try:
                    ...
                    notifications_sent_total.labels(type="inactive_24h").inc()
        ...
        logger.exception("scheduler_job_failed", job="send_inactive_key_notifications")
